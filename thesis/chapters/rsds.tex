...
\workshare{I have collaborated on this work with Ada Böhm; we have both contributed to it equally. I have designed and performed all the experiments described
in this chapter. Source code contribution statistics for \rsds{} can be found
on GitHub\footnoteurl{https://github.com/it4innovations/rsds/graphs/contributors}.}
...
sequential Python code. However, apart from these high-level interfaces, it is also
...
graph that is only expanded on the server lazily, which can help reduce memory and network
...
that tries to minimize the estimated start time of the task. This estimate is based primarily on any
...
communicate directly among themselves to exchange task outputs (data objects) that they require
...
is adopted, it will probably take years before Python packages and programs will fully
adapt to the change.
...
instances on the same node, each running inside a separate process. In this setup, each worker has its
...
\begin{description}[wide=0pt]
	\item[merge-n] creates $n$ independent trivial tasks that are merged at the end (all of
		their outputs are used as input for a final merge task). This benchmark is designed to stress
		the scheduler and the server, because the individual tasks are very short (essentially they do no
		work).
...
were executed twice to lower our computational budget) and averaged the result. We have
...
and medium sized clusters. At worst, it produces a makespan twice as long, but overall
it is close
...
In order to isolate the effects of the specific executed tasks, as well as network communication and
...
This can be examined in more detail in~\Autoref{fig:dask-merge-worker-scaling}, which shows how the
overhead increases when more workers are available. It is clear that the performance of the
work-stealing degrades rapidly when more workers are added to the cluster, while the overhead of the
random scheduler stays almost constant, independent of the number of workers.
...
$1$ ms for most of our benchmarks.
...
take $1000$ ms, $100$ ms and $10$ ms
respectively. With tasks that take a second, \dask{} is able to scale
relatively well up to $1512$ workers. However, when tasks take one tenth of the time, it only
scales up to around $360$ workers, and when tasks take only
$10$ ms, \dask{} scales to around $168$
...
improves significantly, by approx. $35\%$. This makes it clear that the
...
asynchronous \gls{io} and provides strong memory safety guarantees. Therefore, it is
...
generate its messages. However, reimplementing the protocol was relatively challenging, for
...
also performed an experiment where we compare \rsds{} using a random
scheduler with \dask{} using its sophisticated work-stealing scheduler. The
...
serves as evidence that the improved performance of \rsds{} with
...
To provide a clearer comparison of the random scheduler results, Table~\ref{tab:rsds-random-geom-mean-speedup}
...
($360$ workers). This is due to the fact that the cost associated with
...
fares much worse. It is twice as slow when compared to \rsds{} with a single
...
average computation time is still only around $10$ ms, while the average task
...
\dask{} for each task is around $0.3$ ms. In this experiment,
we have measured the per-task overhead of \rsds{}, so that we could qualify how
much its overhead was reduced, relative to the baseline. All the benchmarks in this experiment were
...
how the per-task overhead changes for larger task graphs. From the chart on the left side, we
...
that optimizing the runtime is definitely a worthy effort to pursue, as
