...
in~\Autoref{tab:dask-graph-properties}.
...
\texttt{merge} task graph. However,~\Autoref{tab:dask-graph-properties} shows that the average
...
and furthermore, its performance degrades and eventually becomes slower than the single node case.
...
spectrum, and it reaches up to \SI{1.25}{\milli\second} for some other benchmarks. The results
show that the per-task overhead of \rsds{} is always smaller then the overhead
of \dask{}; however, the difference is not that large for the
