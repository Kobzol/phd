...
This is demonstrated in Listings~\ref{lst:dask-array-example} and~\ref{lst:dask-dataframe-example-2}, which show two
...
in~\Autoref{ch:taskgraphs}. Each task represents a single invocation of a Python function. The
...
Unless otherwise specified, by default we spawn \num{24}
...
This effect can be examined in more detail in~\Autoref{fig:dask-merge-worker-scaling}, which shows how the
...
The fact that \dask{} might struggle with a larger number of workers would not
...
\subsubsection*{The effect of \glsxtrshort{gil}}
...
\begin{figure}
	...
	\caption{Effect of \glsxtrshort{gil} on the performance of the \emph{pandas\_groupby} benchmark}
...
by~\defref{def:dependency_constraint} and~\defref{def:worker_resource_constraint}.
...
the protocol is unfortunately mostly undocumented\footnoteurl{https://github.com/dask/distributed/issues/3357}, and its exact
...
This modification is described in~\Autoref{fig:rsds-dask-frame-encoding}, which depicts a simplified scheme of
...
open-source and available online\footnoteurl{https://github.com/kobzol/distributed/tree/simplified-encoding}. Note that all evaluations presented in
...
described in~\Autoref{sec:rsds-dask-overhead-analysis} demonstrates that it supports a wide variety of
\dask{} programs.
...
this experiment can be seen in~\Autoref{fig:rsds-dask-random-all}. They confirm that the speedup is not
...
be observed in~\Autoref{fig:rsds-random-baseline}. We can see that in the case of
\rsds{}, the random scheduler is less competitive than it was in
\dask{} (cf.~\Autoref{fig:dask-ws-vs-random} and~\Autoref{fig:rsds-random-baseline}) and it does not significantly outperform work-stealing in any of the
...
However,~\Autoref{tab:dask-graph-properties} (in~\Autoref{app:benchmarks}) shows that the average computation time is still only around
...
The results of this experiment can be found in~\Autoref{fig:rsds-zero-worker-speedup}. For all evaluated
