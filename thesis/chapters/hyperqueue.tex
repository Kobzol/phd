...
\begin{itemize}[itemsep=0pt,topsep=2pt]
	...
	%TODO: allow tasks with no resource requirements!
	\item Let $\fntaskresvarcount_{G^{var}}$ be a function that determines the number of
	resource variants for a task of $G^{var}$: \\
	$\fntaskresvarcount_{G^{var}}(t) = \max\limits_{v\in\mathbb{N}_{>0}} \exists
		      r\in\setresourcekindstaskgraph\colon \fntaskresvar(t, r, v) > 0$
\end{itemize}

The remaining properties of $G^{var}$ are identical to the \emph{task graph} properties
