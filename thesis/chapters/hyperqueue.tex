...
\vspace{2mm}\makedef{def:task_graph_execution_var} A \emph{task graph execution with support for
resource variants} is a tuple $E^{var} = (G^{var}, C, \fntaskworkerassigned,
...
\vspace{2mm}\makedef{def:worker_resource_constraint_var} A \emph{task resource variant constraint}
...
a \emph{task graph with support for resource variants} and $C = (W, \setresourcekinds, \fnworkerres)$ is a
