...
We can formally define non-fungible resources by modifying~\defref{def:cluster}
and also~\defref{def:worker_resource_constraint} to add support for resources with identities.

\vspace{2mm}\makedef{def:cluster_ident} A \emph{computational environment with support
...
\begin{itemize}[itemsep=0pt]
	...
	\item $\fnworkerresident$ is a function which defines a set of non-fungible resource elements provided
		  ...
		  In the original definition (\defref{def:cluster}), $\fnworkerres$ assigned a numerical amount to each
	      resource kind. In this  definition, the worker manages a specific set of resource elements instead.
\end{itemize}

\vspace{2mm}\makedef{def:task_graph_execution_ident} A task graph execution with support for non-fungible resources is a tuple $E^{id} = (G, C^{id}, \fntaskworkerassigned, \fntaskresassigned)$, where:
...
\begin{itemize}[itemsep=0pt]
	...
	\item \fntaskworkerassigned{} maps a task to a worker that is executing it at a given point in time; this corresponds to the same definition from~\defref{def:cluster}.
	...
	$\fntaskres$ (as a corollary of \defref{def:worker_resource_constraint}).
\end{itemize}

Using these modified definitions, we can redefine the \emph{worker resource constraint} (originally defined in~\defref{def:worker_resource_constraint}) for non-fungible resources in the context of an execution $E^{id}$:

\vspace{2mm}\makedef{def:worker_resource_constraint_ident} A
...
However, in a following section that describes fractional resource requirements, we will see that there are some
...
\vspace{2mm}\makedef{def:task_resource_constraint_ident} A
...
by~\defref{def:task_graph}, rather than asking for specific resource elements (e.g.\ a task
...
As you may recall from~\Autoref{sec:task-graphs} and~\defref{def:task_graph}, we have
...
\vspace{2mm}\makedef{def:task_graph_frac} A
...
defined in~\defref{def:task_graph}. Note that we can reuse both the task and worker resource
constraints defined in~\defref{def:worker_resource_constraint}, as they still hold even with
...
\vspace{2mm}\makedef{def:task_graph_var} A
...
The remaining properties of $G^{var}$ are identical to the properties of the \emph{task graph}
defined in~\defref{def:task_graph}.

Using~\defref{def:task_graph_var}, we can now also define a modified version of the
\emph{task resource constraint}:

\vspace{2mm}\makedef{def:task_resource_constraint_var} A \emph{task resource variant constraint}
...
time.~\Autoref{sec:hq-resource-management} will describe how can this ambiguity be resolved
...
%elements can be modelled using a combination of indexed pool groups and a \emph{group allocation strategy},
...
%
%\Autoref{fig:hq-cli-task-resources} displays a few examples of how can task resource requirements be
...
%primarly for performance reasons (e.g.\ they might configure allocations to use nodes that reside
...
%manager; they have switched to Slurm in 2023.}. While this approach was working, it
%was also running into some of the challenges presented in~\Autoref{challenge:allocation-manager}; notably, the
...
%non-trivial amount of time performing either serial computation or \gls{io}, during
%which only a few cores were utilized. And with so many cores available on the node, this resulted
%in lower hardware utilization, as many cores remained idle. A simple method of improving
...
%scheduler using the \texttt{taskset} Linux utility. Each group allocation strategy was
%examined; \emph{Compact}, which tries to allocate cores belonging to the same group,
%\emph{Strict compact}, which always draws cores from a single group and \emph{Scatter},
...
%the executed task graph and the four bars represent results of individual strategies. We can see
%that the best performance for $4$ cores per task was achieved with the
%\emph{Compact} strategy, because the threads were able to achieve the highest possible
%memory throughput. In this case, the \emph{Strict compact} strategy achieved the same result;
...
%the achieved performance. The results were similar with $16$ cores per task.
%
...
%
%\subsection{Server \acrshort{cpu} consumption}
...
%	\caption{\hyperqueue{} server \acrshort{cpu} time consumption with an increasing number
%	of tasks}
...
%InfiniBand~\cite{infiniband} network connection, which is also used for network
%communication between the server and the workers; therefore, performing file
%\gls{io} on scratch while also sending many network packets might cause network
%contention.
%
%\begin{table}[h]
%	\centering
...
%large number of files also increases disk usage in general.~\Autoref{tab:hq-io-streaming-size} shows the
%total size of the output of all tasks of the benchmarked task graphs on disk. The
...
%data, writing the output to individual files on disk resulted in around $23\%$
%higher disk usage. In the second configuration, where each task has outputted
%\SI{100}{\kibi\byte} of data, this difference was reduced to $3\%$, because
%the disk space overhead of the files became dwarfed by the size of their content.
%
...
%\rot{Basic (CPU/GPU)} & \rot{Generic resources} & \rot{Non-fungible resources} &
...
%in~\Autoref{ch:sota}. Furthermore, there is no single universally best task runtime; each
%one has a different programming model and a different set of features and trade-offs based on which
...
%define their own resource kinds that are then used in task scheduling. The following four columns
%describe the ability to specify the complex resource management features that were described
%in~\Autoref{sec:heterogeneous-resources}. As was already noted, support for these features is not prevalent in
%existing task runtimes. One notable exception is \ray{}, which supports both
%related resources and fractional resources.
%
%The \emph{Multi-node} column then shows which programming models enable describing
%multi-node tasks. Note that the table only considers this feature as being supported in task
%runtimes that provide the ability to assign the number of nodes \emph{per-task}, not just
%per task graph or per allocation. We can see that \snakemake{} is the only task runtime
%apart from \hyperqueue{} that supports both arbitrary resource requirements and
%multi-node tasks.
%
...
%%execute a predetermined workflow will not be able to achieve such high performance.
%%
