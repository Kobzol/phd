...
($1$ ms, $10$ ms, $100$ ms and
$250$ ms). The \emph{ideal duration} for each task graph was estimated by taking
...
We can see that for tasks that last for $100$ ms or more, the overhead of
...
$1$ ms. Here the overhead of \hyperqueue{} seemingly becomes very large.
...
assumes that the executed command will last for exactly $1$ ms, but from our
...
$1$ ms can take several milliseconds. Because the overhead of actually executing
...
\begin{figure}[h]
	...
	\caption{Strong scalability of \hyperqueue{} with a fixed target makespan ($300$ s)}
...
\begin{figure}[h]
	...
	\caption{Strong scalability of \hyperqueue{} with a fixed task duration ($1$ s)}
