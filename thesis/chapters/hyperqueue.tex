...
\vspace{2mm}\makedef{def:cluster_ident} A \emph{computational environment with support for non-fungible resources} is a tuple
...
\begin{itemize}[itemsep=0pt]
	...
	\item $\fnworkerresident$ is a function which defines a set of non-fungible resource elements provided
	      ...
	      original definition (\defref{def:cluster}), $\fnworkerres$ assigned a numerical amount to
	      each resource kind. In this  definition, the worker manages a specific set of resource elements
	      instead.
\end{itemize}

\vspace{2mm}\makedef{def:task_graph_execution_ident} A task graph execution with support for non-fungible
...
\begin{itemize}[itemsep=0pt]
	...
	\item \fntaskworkerassigned{} maps a task to a worker that is executing it at a given point in time; this
	      corresponds to the same definition from~\defref{def:cluster}.
	\item \fntaskresassigned{} is a function that returns a set of specific resource elements of a
	      ...
	      \defref{def:worker_resource_constraint}).
\end{itemize}

Using these modified definitions, we can redefine the \emph{worker resource constraint} (originally defined in~\defref{def:worker_resource_constraint}) for non-fungible
...
and worker resource constraints defined in~\defref{def:worker_resource_constraint}, as they still hold even with
...
time.~\Autoref{sec:hq-resource-management} will describe how can this ambiguity be resolved through assigning
...
faster was the task running on a \gls{gpu} thatn on the \gls{cpu}.
...
submit tasks and if they support modifying task graphs dynamically. \acrshort{cli}
...
\emph{Python \acrshort{api}} marks runtimes that provide the option to specify tasks using Python. Note
