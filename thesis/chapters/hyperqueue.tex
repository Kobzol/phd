...
into some of the challenges presented in~\Autoref{challenge:allocation-manager}. The submission system was hitting
the limits of the maximum allowed number of allocations that could be submitted by a single user
and it also encountered problems caused by communicating with the allocation manager too
frequently~\cite{atlas-it4i-2}. We have also noticed the latter issue during the implementation
of the \hyperqueue{} automatic allocator described in~\Autoref{hq:automatic-allocation}.

The users of \gls{arcce} have switched to using \hyperqueue{} instead of
submitting allocations directly, in order to resolve these issues~\cite{cern-hq}. Using
\hyperqueue{} resulted in the improvement of the average \gls{cpu}
...
output mode for its \gls{cli} commands, which facilitates this kind of usage.
