...
existing task runtimes. However, while performance is naturally crucial for \gls{hpc}, it should not
...
thesis, namely the ergonomics of task graph execution.
...
\begin{enumerate}
	...
	\item We provide an implementation of the meta-scheduling design in the
	      \hyperqueue{} task runtime, which enables efficient execution of task graphs on
		  heterogeneous \gls{hpc} clusters. It also supports dynamic load balancing
		  features that are not present in other state-of-the-art tools, such as fractional
		  resources and resource variants.
