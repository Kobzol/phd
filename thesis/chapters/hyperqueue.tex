...
\begin{enumerate}
	...
	\item We provide an implementation of the meta-scheduling design in the \hyperqueue{} task
	      ...
	      clusters.
%		  It also supports dynamic load balancing features that are not present in other
%	      state-of-the-art tools, such as fractional resources, resource variants and identity resources.
...
In addition to these general guidelines, we will also describe a novel approach for managing heterogeneous
resources.

\section{Heterogeneous resource management}
\label{sec:heterogeneous-resources}
As was already discussed in~\Autoref{challenge:heterogeneity}, support for heterogeneous
...
make sure (in accordance with~\defref{def:worker_resource_constraint}) that it executes a single
...
We can formally define non-fungible resources by
modifying~\defref{def:worker_resources}
and~\defref{def:worker_resource_constraint} to add support for resources
with identities.
...
\begin{itemize}[itemsep=0pt]
	...
	each resource kind (\defref{def:worker_resources}). To support non-fungible resources, we
	...
	would be identical to the $\fntaskres$ function (\defref{def:task_resreq}).
	...
	trivial consequence of~\defref{def:worker_resource_constraint}; however, that is no longer
...
%	Recall that our original assumption was that all tasks were executed exactly once in $X$;
...
as defined by~\defref{def:task_resreq}, rather than asking for specific resource elements (e.g.\
...
However, in certain cases it can be useful for a task to ask for a set resource elements that
...
designed and implemented in various ways.~\Autoref{sec:hq-resource-management} will describe how
is it implemented in \hyperqueue{}.

\subsection{Fractional resource requirements}
As you may recall from~\Autoref{sec:task-graphs} and~\defref{def:task_resreq}, we have defined a resource
...
\defref{def:task_resreq} as follows:

\resourcerequirement{\fntaskresfrac}{Q}

%TODO: generalize theory

Support for fractional resource requirements can be useful to express situations
...
in~\Autoref{challenge:allocation-manager}, using Slurm allocations directly as tasks causes
several issues.
...
required by each task, as was defined by~\defref{def:task_resreq}. This resource requirement is
...
\begin{itemize}[itemsep=0pt]
	...
	\resourcerequirement{\fntaskresvariant{i}}{Q}
	\item We can now define a modified version of the \emph{task resource constraint}
	(\defref{def:task_resource_constraint}) that takes resource variants into account: \\
...
What makes the scheduler different from \rsds{} is its support for the complex resource
management scenarios that were described in~\Autoref{sec:heterogeneous-resources}.
...
using resource variants will be evaluated in~\Autoref{sec:hq-exp-resource-variants}.
...
described in~\Autoref{sec:dynamic-resource-selection}. For evaluation, we use the simple workflow
