...
\workshare{I have collaborated on this work with Ada Böhm; we have both contributed to it equally. \\
I am the sole author of the design and implementation of the automatic allocator
...
\begin{description}[wide=0pt]
	...
	\item[Allocations are submittable automatically] A corollary of the previous principle is that it becomes possible to submit allocations fully
		...
		allocations.
	% \\ to fix overfull hbox
	\item[Failed tasks are retried automatically] Once allocations become generic computational \\
	    providers, their lifetime is no longer bound to a
...
\begin{itemize}[itemsep=0pt]
	...
	\item Let $\setresourceelements_{C^{id}}$ be a set of all resource elements of $C^{id}$: \vspace{1mm}\\
	      $\setresourceelements_{C^{id}} = \bigcup\limits_{r\in\setresourcekindsident} r$
	\item $\fnworkerresident\colon W \times \setresourcekindsident \rightarrow \powerset{\setresourceelements_{C^{id}}}$ is a function that defines a set of non-fungible resource elements provided by a worker for a specific resource kind.
		  ...
		  of unique resource elements; therefore, $\fnworkerresident$ returns a set of
		  elements instead.
	\item $\fnworkerresident$ has to return resource elements belonging to the correct resource
	kind: \vspace{1mm}\\
		$\forall w\in{}W, \forall r\in\setresourcekindsident: \fnworkerresident(w, r) \subseteq r$
	\item Workers do not share resource elements between each other: \vspace{1mm}\\
	...
	\item Workers do not share resource elements between different resource kinds: \vspace{1mm}\\
...
This definition describes a computational environment where each worker manages a set of unique
resource elements that are non-fungible; each one has a separate identity and cannot be
interchanged with another resource element.
...
\begin{itemize}[itemsep=0pt,topsep=2pt]
	...
	\emph{computational environment with non-fungible resources}.
	\item $\fntaskworkerassigned$ is a function that maps a task to a worker that is executing it
	at a given point in time; this corresponds to $X$ defined in~\defref{def:task_graph_execution}.
	\item $\fntaskresassigned\colon \settasks \times W \times \setresourcekindsident \times \timedomain \rightarrow
	\powerset{\setresourceelements_{C^{id}}}$ is a function that returns a set of specific resource elements of a
		  ...
		  (\defref{def:cluster}) we did not
		  need the $\fntaskresassigned$ function to define worker and task resource constraints
		  (\defref{def:worker_resource_constraint}); $\fntaskres$ was used instead. However,
		  that is no longer enough in the presence of non-fungible resources.
	\item When a task is not being executed at a given point in time, no resource elements are
	assigned to it: \vspace{1mm}\\
	$\forall tp\in\timedomain, \forall t\in\settasks, \forall w\in{}W, \forall r\in\setresourcekindsident\colon X(t, tp) = \bot \Rightarrow \fntaskresassigned(t, w, r, tp) = \emptyset$
	\item Resource elements allocated to a task that is being executed have to be drawn from the
	correct resource kind: \vspace{1mm}\\
	$\forall tp\in\timedomain, \forall t\in\settasks, \forall r\in\setresourcekindsident\colon X(t, tp) \neq \bot \Rightarrow \fntaskresassigned(t, X(t, tp), r, tp) \subseteq r$
\end{itemize}
...
defined as follows: \vspace{1mm}\\ $\forall tp\in\timedomain, \forall w\in{}W, \forall
	r\in{}\setresourcekindsident\colon \left(\bigcup_{t\in{}\fnworkertaskassigned_{E^{id}}(w, tp)}
	\fntaskresassigned(t, w, r, tp)\right) \subseteq \fnworkerresident(w, r)$

\vspace{2mm} This property ensures that resource elements assigned to a task belong to the
worker that is executing it. If we want to also ensure that a given resource element is not being oversubscribed, i.e.\ that
is used by at most a single task at any given point in time, we could also add the following constraint: \vspace{2mm}\\
$\forall tp\in\timedomain, \forall w\in{}W, \forall
	r\in{}\setresourcekindsident, \forall t_1\in{}\settasks, \forall
...
\vspace{1mm}This property ensures that at any given point in time when it is being executed, each
task has been assigned the required amount of resource elements for each resource kind. Note that this
definition allows the individual resource elements assigned to tasks to change over time; some
task runtimes might leverage this to implement a very dynamic form of resource management.
...
Taking resource identity into account improves \ergonomics{}, because it makes it easier for
...
is a computational node with \num{128} \gls{cpu} cores and we want to
...
designed and implemented in various ways.~\Autoref{sec:hq-resource-management} will describe how it is
...
Specifying related resources is designed to improve \efficiency{}, as it should allow the
...
\vspace{2mm}\makedef{def:task_graph_frac}{Task graph with fractional resource requirements} A
\emph{task graph with fractional resource requirements} is a tuple \\
$G^{fr} = (\settasks, \setobjects, \setarcs, \setresourcekindstaskgraph, \fntaskresfrac)$, where:
\begin{itemize}[itemsep=0pt,topsep=2pt]
	\item \resourcerequirement{\fntaskresfrac}{Q} is a function that defines the
	      \emph{fractional resource requirement} of a task for a given resource kind.
\end{itemize}

The remaining properties of $G^{fr}$ are identical to the \emph{task graph} properties
defined in~\defref{def:task_graph}. Note that we can reuse both the task
and worker resource constraints provided by~\defref{def:worker_resource_constraint} for
\emph{task graphs with fractional resource requirements}, as they still
hold even with fractional resource requirements if we simply replace $\fntaskres$ by
$\fntaskresfrac$.
...
\num{16} \gls{cpu} cores in the case that a \gls{gpu} is
...
specify multiple \emph{resource variants} of resource requirements, where each variant is a
separate set of resource requirements. The task runtime can
then dynamically choose which variant to use for a task based on current computational load and
...
$G^{var} = (\settasks, \setobjects, \setarcs, \setresourcekindstaskgraph, \fntaskresvar)$, where:
\begin{itemize}[itemsep=0pt,topsep=2pt]
	\item $\fntaskresvar\colon \settasks \times \setresourcekindstaskgraph \times
	\mathbb{N}_{>0} \rightarrow
	\mathbb{N}_{\geq{}0}$ is a function that defines the resource
	      requirement of a task for a given resource kind and a given resource variant index.

	The individual variants are identified with consecutive numeric indices starting at \num{1}.
	\item Let $\fntaskresvarcount_{G^{var}}$ be a function that determines the number of
	resource variants for a task of $G^{var}$: \vspace{1mm}\\
	$\fntaskresvarcount_{G^{var}}(t) = \begin{cases}
	   1,&\text{if } \forall v\in\mathbb{N}_{>0}, \forall r\in\setresourcekindstaskgraph\colon \fntaskresvar(t, r, v) = 0 \\
	   \max\limits_{v\in\mathbb{N}_{>0}} \exists
	   r\in\setresourcekindstaskgraph\colon \fntaskresvar(t, r, v) > 0,&\text{otherwise}
	\end{cases}$

	If a task does not define any resource requirements,
	then $\fntaskresvarcount_{G^{var}}(t) = 1$. In that case, the task behaves as if
	it had a single variant that does not require any resources.
\end{itemize}

\vspace{2mm}The remaining properties of $G^{var}$ are identical to the \emph{task graph} properties
defined in~\defref{def:task_graph}.
...
\begin{itemize}[itemsep=0pt,topsep=2pt]
	\item $G^{var} = (\settasks, \setobjects, \setarcs, \setresourcekinds, \fntaskresvar)$ forms a
	\emph{task graph with resource variants}.
	\item $C = (W, \setresourcekinds, \fnworkerres)$ forms a \emph{computational environment}.
	\item $\fntaskworkerassigned$ is a function that maps a task to a worker that is executing it
	at a given point in time; this corresponds to $X$ defined in~\defref{def:task_graph_execution}.
	\item $\fntaskvarassigned\colon \settasks \times \timedomain \rightarrow
	\mathbb{N}_{>0}$ is a function that returns a specific resource variant
	...
	$\forall tp\in\timedomain, \forall t\in{}\settasks\colon \fntaskworkerassigned(t, tp) \ne \bot \Rightarrow
	...
	is allowed to return an arbitrary non-zero natural number.
\end{itemize}

Using the two previous definitions, we can now also define a modified version of both the
worker and task resource constraints:

\vspace{2mm}
\makedef{def:worker_resource_constraint_var}{Worker and task resource constraint with resource variants} A
\emph{task resource constraint with resource variants}
and a \emph{worker resource constraint with resource variants} in the context of execution $E^{var} = (G^{var}, C, \fntaskworkerassigned, \fntaskvarassigned)$, where $G^{var} = (\settasks, \setobjects, \setarcs, \setresourcekinds, \fntaskresvar)$ is
a \emph{task graph with resource variants} and $C = \clusterinner{\setresourcekinds}$ is a
\emph{computational environment}, are defined as follows: \vspace{1mm}\\
$\forall tp\in\timedomain, \forall w\in{}W, \forall
r\in{}\setresourcekinds\colon \left(\sum_{t\in{}\fnworkertaskassigned_{E^{var}}(w, tp)} \fntaskresvar(t, r, \fntaskvarassigned(t, tp))\right) \leq \fnworkerres(w, r)$
...
displays a high-level view of the \hyperqueue{} architecture for a typical deployment on
...
contains all the relevant code inline; it is thus self-contained and does not
...
information necessary for exchanging handshakes and connecting the components, including
encryption keys. When workers and
...
\hyperqueue{} terminology. Each task belongs to exactly one job; jobs are completely
...
ID \num{1}, the second one will have ID \num{2}, etc. The ID is
passed to the task through an environment variable. Apart from assigning unique inputs to tasks
through task IDs, \hyperqueue{} also supports passing a unique binary blob to each task
...
\begin{description}[wide=0pt,itemsep=0pt,topsep=4pt]
	...
	\item[Finished] When a task finishes successfully (a binary program exits with the exit code \num{0}
...
output streams, or to let \hyperqueue{} remove them if they are empty after the task has
...
into limitations caused by user disk quotas. To alleviate this problem, \hyperqueue{}
...
%~\Autoref{lst:hq-cli-log} shows several \hyperqueue{}
...
\hyperqueue{} server is based on the \rsds{} server implementation, which
supports data transfers between workers, so the transfer functionality itself is actually
implemented. However, data transfers still need to be incorporated into
...
jobs, as they exist only in the frontend. The scheduler ensures that the
\emph{dependency constraint} (\defref{def:dependency_constraint}) is always upheld.

What makes the scheduler different from \rsds{} and other state-of-the-art task runtimes is its
support for the heterogeneous resource management concepts described
in~\Autoref{sec:heterogeneous-resources}. It supports
...
arbitrary resource kinds for both tasks and workers. At the same time, workers also
recognize several known resource kind names and provide specialized support for them. Resource
...
Worker resources are configured when a worker is started through so-called \emph{resource pools}, which
represent individual resource kinds (named sets of resource elements). The most common resource
pools, such as \gls{cpu} cores and their \gls{numa} sockets, the amount of \gls{ram} and the
...
\begin{description}[wide=0pt]
	\item [Indexed pool] represents a set of resource elements where each individual element is
	      ...
	      an indexed pool containing two elements (e.g.\ with indices \num{0} and
	      \num{1}), each representing one individual \gls{gpu} device. These two
	      ...
	      When a worker is started, by default it automatically divides available \gls{cpu} cores
		  into groups according to their \gls{numa} nodes. However, the concept of groups is
		  general; they can be used for any indexed pool, not only specifically for \gls{numa}.

		  The scheduler ensures that the
	\emph{worker resource constraint with non-fungible resources}
		  (\defref{def:worker_resource_constraint_ident}) is upheld for indexed pools.
	\item [Sum pool] is designed for resources that are fungible. Such resources are typically
		  ...
		  The scheduler ensures that the
	\emph{worker resource constraint}
	(\defref{def:worker_resource_constraint}) is upheld for sum pools.
\end{description}
...
\begin{description}[wide=0pt,itemsep=0pt,topsep=1pt]
	\item[Fractional resource requirements] Tasks can specify
...
When a task has a fractional resource requirement that is drawn from an indexed resource
...
leverage more than one resource element of a given resource kind.
	...
	\item[Resource variants] Each task can define a list of
...
be evaluated in~\Autoref{sec:hq-exp-resource-variants}.
	...
	\item[Related resources] Relations between resource
...
\emph{time request}. If a task specifies a time request (a duration) $tr$, it
tells the scheduler that it needs at least $tr$ time units for its execution.
...
asks for two \gls{cpu} cores and the scheduler allocates cores \num{10}
and \num{14} of a specific worker to it, \hyperqueue{} will pass the
...
are used only for scheduling. For example, if a task is provided with cores \num{0}
and \num{1}, the worker that executes it does not enforce that it will actually run
...
\gls{cpu} cores, \hyperqueue{} can pin the allocated cores for the task
...
multiple workers at the same time. In this case, \hyperqueue{} assumes exclusive
...
same time. When a multi-node task starts to execute, \hyperqueue{} passes it a
...
\gls{cli} to resubmit failed tasks from a previously
submitted job. It also shows that several \hyperqueue{} commands are designed to be
...
the whole job will be preemptively canceled to avoid wasting computation.
...
implemented in \hyperqueue{} under the name \emph{Automatic allocator} (shortened as \autoalloc{}). The automatic
...
be submitted when there is a demand for more computational resources.
...
\begin{description}[wide=0pt,itemsep=0pt,topsep=2pt]
	\item[Allocation manager] \Autoalloc{} needs to communicate with a given allocation manager
		to be able to submit allocations into it. Users thus need to tell \hyperqueue{} which
	...
	\item[Worker count per allocation] Determines how many nodes (workers) should be requested for each allocation. Unless users want to
		...
		(\num{1}); typically, the fewer nodes are requested in an allocation, the sooner it
...
$ hq alloc info 1
	\end{minted}
	\caption{Configuring automatic allocation using the \hyperqueue{} \glsxtrshort{cli}}
...
adopting these approaches would require \hyperqueue{} to introduce a dependency on Python
...
information can be learned through a proxy; the connections and disconnections of
workers. When a worker is started in an allocation, it will connect to the
...
The first two use-cases originate from the LIGATE (LIgand Generator and portable drug discovery
...
experiments on petascale and exascale \gls{hpc} clusters. \hyperqueue{} was created within the
LIGATE project; its use-cases served as one
of the main driving forces behind the design of this task runtime. The usage of \hyperqueue{}
...
task resource requirements, which allowed selecting the proper hardware
...
simplicity of porting \hyperqueue{} workflows to different supercomputers.
...
database; for each complex, up to \num{256} different poses were evaluated, and for
each pose, \num{8} simulation replicas were executed to reduce statistical error
...
simulations) for a given complex pose, either on \num{8} \glspl{gpu} or
\num{16} \gls{cpu} cores in parallel. \hyperqueue{} thus had
to schedule and manage approximately \num{800} thousand tasks in total. The automatic
...
intervention. This was possible due to the fact that the server was
...
\num{240} thousand \gls{gpu} hours on \emph{LUMI-G}
(the \gls{gpu} partition of the LUMI~\cite{lumi} cluster) and
\num{2} million \gls{cpu} core hours on the
...
into a text file (where each configuration is a separate line), from which \hyperqueue{}
can then automatically build the workflow.~\Autoref{lst:hq-exa4mind-hyperparameter-search} shows an example of how easy is
to use \hyperqueue{} in this case. The user simply starts the server, configures
...
which had \num{24} \gls{cpu} cores per node. This was a reasonable
...
Karolina~\cite{karolina}, which has \num{128} cores per node instead. This
...
\gls{arcce} switched to it, the computations were configured for \num{32}
...
In selected experiments, \hyperqueue{} was benchmarked in a so-called
...
We have benchmarked the \emph{simple workflow} in two sizes (\num{10} and
\num{50} thousand tasks) with three different worker counts (\num{1},
\num{2} and \num{4} workers) and four different task durations
...
dividing it by the total number of cores available (worker count $\times$ \num{128}). This estimates a
...
task graph that contains \num{50} thousand tasks, the overhead stays within
...
\hyperqueue{}, in parallel on all \num{128} available cores. This provides a
...
processed per second) using the \emph{zero worker} mode. In this mode, \hyperqueue{}
performs all operations that it does normally (managing tasks and workers, scheduling, sending
network packets), but workers do not actually execute tasks; the corresponding worker
immediately marks each task as completed instead. This allows us to examine the overhead of
...
\emph{simple workflow} containing from \num{10} thousand to \num{1}
million tasks with varying worker counts (up to \num{16} workers and thus
\num{2048} \gls{cpu} cores).
...
the smallest task graph with \num{10} thousand tasks, the throughput increases up to
four workers (\num{512} cores), then it decreases; the task graph is too small and the
constant overhead factors start dominating over the available parallelism. For task graphs with up
to \num{100} thousand tasks, the throughput also increases up to four workers, where it
...
for a task graph with \num{50} thousand tasks and a single node (using the
...
\num{64} workers and thus \num{8192} cores in total). Note that each
...
minutes (\num{300} seconds) on a single worker, and benchmarked
...
each graph, so that the total makespan on a single worker would be \num{5} minutes.
The benchmarked worker counts were \num{1}, \num{2},
\num{4}, \num{8}, \num{16}, \num{32}
and \num{64}.
...
durations, \hyperqueue{} was able to scale reasonably well up to \num{64}
workers, where it was able to compute the whole task graph in approximately \num{5}
seconds. In the best case, with tasks that took \SI{2.5}{\second} to execute, it provided a
$59.2$x speedup over a single worker using \num{64} workers. In the worst case, with
tasks that lasted \SI{100}{\milli\second}, it provided a $54.4$x speedup over a single worker using
\num{64} workers. With task duration \SI{0.1}{\second} and
\num{64} workers, \hyperqueue{} was able to dispatch almost
\num{70} thousand tasks per second.

In the second scenario, we fixed the task duration of each task to \num{1} second
...
with the number of tasks; with \num{100} thousand tasks, \hyperqueue{} is
able to provide a $59$x speedup with \num{64} workers. On the
other hand, with only \num{10} thousand tasks, the speedup in this case reaches only
$38.8$x, because there are not enough tasks to saturate all workers. With
\num{64} workers that each have \num{128} cores, the total number of
cores managed by \hyperqueue{} is \num{8192}. Therefore, each worker barely gets a single task to compute in
...
\subsection{Performance comparison of \dask{} and \hyperqueue{}}
...
separate worker configurations, with \num{1} process per node and
\num{128} threads per process ($1p/128t$), with \num{8}
processes per node and \num{16} threads per process ($8p/16t$) and
with \num{128} processes per node and \num{1} thread per process
($128p/1t$). The reason for choosing different process/thread combinations has been
extensively explained in~\Autoref{dask:evaluation}; we chose two extremes (\num{1}
process and \num{128} processes) and then a compromise with \num{8}
...
makespan of \num{30} seconds on a single node). The horizontal axis shows the amount
...
the task graph and also on the task duration. With only \num{1000} tasks and task
...
in the benchmark with \num{50000} tasks. With a single node, \dask{} is
...
\hyperqueue{} worker manages all resources of its node, it does not require
...
computational node has \num{128} cores and \SI{256}{\gibi\byte} of memory, divided
into \num{8} \gls{numa} nodes. Accessing memory across these
...
We have benchmarked the \emph{simple workflow} with \num{100} tasks on a single
Karolina node, where each task executes the described program with either \num{4}
or \num{8} threads (cores). Each task is configured so the spawned threads are
...
simple hyperparameter search that executes \num{20} independent tasks. Each task executes a Python
...
that can achieve over \num{300} teraFLOPS (trillion floating-point operations per
...
single \gls{gpu}; for example, with a requirement of $0.1$ \glspl{gpu}, \num{10} tasks can run on the
same accelerator at once. The requirement of \num{1} \gls{gpu} per task emulates the behavior of a
...
\begin{figure}[h]
	...
	\caption{\glsxtrshort{gpu} hardware utilization improvements with fractional resource requirements}
...
displayed in~\Autoref{fig:hq-fractional-resources}. The vertical axis shows the average
...
\num{300} tasks executed on a single node with \num{8} virtual
...
\num{10} thousand tasks, where each task fully utilizes a full
...
over all \num{128} cores for each separate worker. The red horizontal dotted line marks the
...
\num{5} seconds. In this case, \hyperqueue{} workers kept their average
...
workflow execution, where approximately \num{30} seconds before the end, the
...
opportunities for load balancing performed by \hyperqueue{} and there might not be enough
...
an input containing \num{200} thousand ligands. The computation duration varies
significantly based on the length of each ligand; therefore, we have tested two input files. The
first one ($uniform$) contains \num{200} thousand copies of the same
...
single-threaded. For \num{5000} ligands per file (the rightmost column), the total
number of expansion tasks is only \num{40}; therefore, the achieved utilization (for
both input files) is only approximately $32\%$, which corresponds to using just
\num{40} out of the \num{128} available cores. With more than
\num{128} files available, the utilization approaches $100\%$ for the
uniform input. For the skewed input file, the utilization for the expansion section is in general
worse than with the uniform input, as expected. With \num{100} ligands per file, it
...
The scoring part is internally parallel and uses all \num{8} assigned cores;
therefore, it does not have to be divided into so many tasks. We can see that even with just
\num{40} files it achieves approximately $80\%$ utilization for
...
section with \num{2000} files reaches only approximately $63\%$ with
...
\num{1000} ligands per file, the total average utilization for both workloads is at or
...
how does the workflow scale up to \num{4} workers (\num{512} cores).
...
\num{400} ligands per task starts to beat the configuration with
\num{1000} ligands per task, simply because the latter configuration is not able to
...
executed using \num{16} ligands per file and \num{8} threads per
file on an input file containing \num{24} thousand ligands with a skewed
...
period. The server was running on a login node and it was managing up to \num{12} workers
(\num{1536} cores) and up to \num{200} thousand tasks. Each worker was
...
server changes with a fixed number of workers (\num{12}) and an increasing number of
tasks. In this case, the server had to schedule the \emph{simple workflow} with the number of tasks
ranging from \num{10} to \num{200} thousand. The horizontal axis shows
...
\num{200} thousand tasks within this short period, which is an extreme stress test
...
\num{1000} tasks was approximately \SI{0.0003}{\second}. In other words, for every
...
is fixed (\num{50000}), but the number of workers increases from
\num{2} to \num{12}. The horizontal axis shows the number of workers
...
\num{200} thousand tasks, the memory consumption of the server, measured using the Linux
...
\num{10}, \num{50} and \num{100} thousand tasks executed
...
case with \num{100} thousand tasks, the makespan is approximately twice longer with
...
filesystem. We have benchmarked two task graphs without dependencies, with \num{10}
and \num{50} thousand tasks, where each task generates a fixed amount of data
(\num{10} or \SI{100}{\kibi\byte} per task). Each task simply outputs the
...
\newcolumntype{E}{c@{}c@{}c@{}c@{}c@{}c@{}c}
\begin{threeparttable}
\begin{tabular}{|l|C|A|A|B|E|A|l|}
...
%A brief description of each compared task runtime can
...
the heterogeneous resource management concepts introduced in~\Autoref{sec:heterogeneous-resources}
are able to both improve the achieved hardware utilization of heterogeneous resources and also
decrease the makespan of executed task graphs.

Several ideas for possible future improvements of \hyperqueue{} will be summarized in the final
chapter.
