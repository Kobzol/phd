...
\begin{tablenotes}
	\centering
	\item \yes{} supported; \no{} not supported; \automaticmap{} automatic; \manualmap{}
manual; \textsuperscript{\dag}with an external plugin
\end{tablenotes}
...
The third section, \emph{Meta-scheduling}, specifies four properties related to executing tasks
...
\begin{description}[wide=0pt]
	...
	\item[Fault tolerance] describes what happens to tasks that fail because of an external
	(e.g.\ worker node) failure. Task runtimes that can automatically restart such tasks are
	...
	kind of fault tolerance; it is a truly fundamental feature without which it would be
	infeasible to execute task graphs.
...
see that almost all evaluated task runtimes require Python, it is a very popular technology for
working with tasks. Some task runtimes, such as \merlin{} or \fireworks{}, additionally require
external service for managing persistence or task assignment, these can thus be challenging to
deploy on \gls{hpc} clusters.
