...
job managers, distributed job schedulers or orchestrators. We will use the term
...
scalability, fault tolerance, data provenance, ease-of-use, ease-of-deployment and others.

Note that there is a set of task-based tools that are commonly labeled as
\emph{Workflow Management Systems}, for example Apache Airflow~\cite{airfow},
Dagster~\cite{dagster}, Prefect~\cite{prefect} or
Argo~\cite{argo}. Although it cannot be said that there is a single distinguishing
feature that would separate these tools from other task runtimes, from my point of view these tools
form a separate category. They are primarily designed for very coarse-grained workflows with a
relatively small number of tasks, they put strong emphasis on workflow reproducibility, data
provenance, execution monitoring (typically through a web interface) and support scheduled
execution (i.e.\ ``run this workflow every hour''). They are also typically being applied in cloud
environments, moreso than on supercomputers. This thesis will not consider these tools in great
detail, as they are not primarily designed for \gls{hpc} use-cases.
...
\Autoref{hq:related-work} will describe several task runtimes in more detail, and compare them
with the proposed \hyperqueue{} tool.
...
\begin{enumerate}
	...
	\item The allocation manager puts the request into a queue and schedules it to be executed at some time
	      ...
	      will an allocation run, the schedule can be very dynamic and unpredictable, so users might have to
...
before receiving first results (finished tasks).
...
graph into \emph{groups}, where all tasks of a single group are always executed
inside the same allocation. However, this partitioning has to be specified explicitly by the author
of the workflow, which can be laborious.
