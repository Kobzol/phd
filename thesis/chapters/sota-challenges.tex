...
% TODO: remove AiiDa/NextFlow/StreamFlow?
...
StreamFlow~\cite{streamflow}, AiiDA~\cite{aiida} or
...
primarily from the complexity and idiosyncracies of supercomputers, which have been historically
...
research~\cite{glume,slurm-workflow}, and it even led to proposals for making existing
allocation managers workflow-aware~\cite{slurm-workflow} or even creating new allocation
managers that have better support workflows~\cite{flux}.
...
therefore it is easier for the manager to schedule them. Similar behavior can be observed w.r.t.\
...
runtimes.~\Autoref{ch:hyperqueue} then also deals with the ergonomical challenges, and proposes
