In order to design approaches for seamless execution of task graphs on supercomputers, it is first necessary to examine
...
After the general overview of existing task runtimes presented in this chapter,~\Autoref{ch:rsds} will
provide a detailed analysis of \dask{}, a state-of-the-art task runtime,
and~\Autoref{hq:related-work} will then compare several existing task runtimes with \hyperqueue{}, which will be the main
topic of~\Autoref{ch:hyperqueue}.
...
lead to hardware underutilization and resource waste~\cite{ehpc}. Consider a typical data
...
\gls{gpu} accelerator. To execute such a workflow in a single allocation, we will need to ask for a set
...
in which these nodes will be underutilized or completely idle. This is caused by the fact that all
...
This approach can be used with essentially all existing task
...
one go. This method is thus relatively simple for the user, although as has been mentioned, it
...
overhead per each allocation, if not more, which is order(s) of magnitude more than the overhead of an
efficient task runtime~\cite{rsds}.
...
same time (some efforts have been made on this front e.g.\ by Flux~\cite{flux}, which will be described later below),
it is probably infeasible to modify the two most prominent allocation managers that are used almost
...
Another reason why using an allocation manager directly as a task runtime might be impractical is that
...
For example, the intermediate outputs of
...
resubmit them into new allocations.
...
this process. We will use the term \emph{meta-schedulers} for these task runtimes in this
thesis. These tools typically operate on a level above the allocation manager, instead of only
running within the context of a single allocation. They are thus able to manage task graphs that
span multiple allocations.

Existing task runtimes use various approaches for providing some form of meta-scheduling. Tools
such as \gnuparallel{}~\cite{parallel} can automatically map each task to a separate allocation.
While this can be useful for running simple task graphs on supercomputers, it does not help overcoming
allocation manager submission limits. \hypershell{}~\cite{hypershell} improves upon this by enabling
users to specify a \emph{task bundle size} parameter, which batches the configured number of
tasks together in the same allocation. While this method can help amortize the cost of
allocations for large task graphs, it is mostly only useful for task graphs without dependencies,
for which it is possible to express the partitioning using a single number. It can be used by
\hypershell{} since it does not support dependencies between tasks.

\snakemake{}~\cite{snakemake} allows its users to explicitly assign each task to a
\emph{group}. Tasks with the same group are then computed in the same allocation.
...
allocation, which is actively discouraged on some \gls{hpc} clusters~\cite{nersc-snakemake}.

Task runtimes such as \pegasus{}~\cite{pegasus} or \autosubmit~\cite{autosubmit} slightly improve
...
task runtime, which will be described in~\Autoref{ch:hyperqueue}.
...
\begin{description}[wide=0pt]
	...
	algorithms will be examined in detail in~\Autoref{ch:estee}.
	...
	runtime implemented of Python, will be examined in detail in~\Autoref{ch:rsds}.
	...
	tasks are generated from a single location, the bottleneck will most likely remain even in an
	...
	network card. This line of research is not pursued in this thesis, although it could
