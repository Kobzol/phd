...
The following two chapters have focused on the performance and efficiency aspects of executing task
...
scheduling algorithms and examine several hypotheses the effect of various factors on the
...
We have followed this line of thought in~\Autoref{ch:rsds}, which has focused on the
...
the ergonomical aspects of designing and executing task graphs on supercomputers. It has introduced
...
The following list describes how does \hyperqueue{} deal with challenges introduced
...
\begin{description}[wide=0pt]
	\item[Interaction with allocation manager] The used meta-scheduling approach removes the need for the workflow author to think about mapping
		...
		could be extended with task duration and/or allocation start time predictions, to improve its
		decisions on when to actually submit allocations.
	\item[Cluster heterogeneity] \hyperqueue{} provides comprehensive support for heterogenenous
	...
	\item[Data transfers] \hyperqueue{} does not currently support direct data transfers
		...
		network communication), which limits its applicability (or at least its ergonomics) in scenarios
		...
		standard (error) output files on distribtued filesystems, \hyperqueue{} offers
	...
	\item[Fault tolerance] \hyperqueue{} offers fault tolerance by default; tasks that do not
		finish computing successfully due to reasons outside of their control are automatically rescheduled
	...
	\item[Deployment] In terms of ease-of-deployment, \hyperqueue{} is essentially optimal; it is
		...
		it. It is even possible to easily make it completely self-contained by statically linking a
...
efficiency of existing \dask{} workflows, we have contacted the authors and
...
implemented in \rsds{} have been since adapted in the \dask{}
project, which helped alleviate some of the bottlenecks that we have discovered with our
...
In addition to being used directly by users as a task runtime, \hyperqueue{} can also
...
hardware utilization by 30\%~\cite{cern-hq} on the IT4Innovations Karolina and Barbora
clusters. \hyperqueue{} has also been used to execute workflows in several projects
...
that it has originally set out. I am confident that \hyperqueue{} provides a tangible
benefit in terms of ergonomical and efficient execution of task graphs on supercomputers, and that
