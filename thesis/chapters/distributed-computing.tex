...
\begin{description}
	...
	\item[Shared-memory multiprocessing] is an approach that focuses on the parallelization within a single computing node, by leveraging
		...
		in~\Autoref{lst:openmp-annotation}, where a loop is parallelized simply by adding a single annotation to
		the source code.
...
relevant to task-based programming models, and~\Autoref{ch:sota} will then describe challenges
