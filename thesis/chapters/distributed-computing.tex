...
and thus achieving speedup through parallelisation.
...
that express parallelisation and network communication explicitly, and models that do so
implicitly.
...
parallelisation of computation and the exchange of data between nodes explicitly. This has been the
...
\begin{description}
	...
	\item[Shared-memory multiprocessing] is an approach that focuses on the parallelisation within a single computing node, by leveraging
		...
		with \gls{mpi}~\cite{hybrid_openmp_mpi}), in order to achieve parallelisation both
		...
		source code annotations (called \emph{pragmas}), which allow the programmer to parallelise
		a region of code with very little effort. An example of this can be seen in \Autoref{lst:openmp-annotation},
		where a loop is parallelised simply by adding a single annotation to the source code.
...
cores and nodes cooperate also enables expressing arbitrarily complex parallelisation patterns and
data distribution strategies. However, in order to fully exploit the performance potential of
explicit parallelisation, the programmer must have advanced knowledge of the \gls{cpu}
...
Even though explicitly parallelised programs can be incredibly efficient, implementing correct
...
\subsection*{Implicit parallelisation}
...
(explicitly parallel) code for them from scratch. Instead, high-performance parallelised primitives
...
domain, as their responsibility shifts from implementing communication and parallelisation
techniques by hand to describing high-level computational workflows using \emph{implicity parallel}
programming models that are able to automatically derive the communication and parallelisation
structure from the program description. With an implicitly parallel model, the emphasis moves from
\emph{how} to perform a distributed or parallelised computation (which is the essence
...
paradigms, declarative and imperative. The possible parallelisation opportunities are expressed
...
computation that can be automatically parallelised, without forcing the user to think about how
exactly will the parallelisation and network communication be performed. Execution frameworks are
...
than the explicit models, and they facilitate rapid prototyping of parallelised programs.

On the other hand, the main disadvantage of these models is the lack of control of how exactly is
parallelisation performed. Therefore, programs implemented using them might not be able to achieve
the same performance as explicitly parallelised programs.

There are various models that support implicit parallelisation, for example stencil
computations~\cite{stencil} or automatically parallelised functional
...
imperatively specify how should their computation be parallelised, or when and how should data be
...
user to explicitly define how should the program be parallelised.
...
structure of the task. Therefore, the only parallelisation opportunities that can be extracted by
...
parallel, however this parallelisation is not provided automatically by the task runtime. Tasks
...
automatically parallelised and distributed. Furthermore, the ``layered'' shape of task graphs
...
Specialized types of task graphs are also commonly used to parallelise \emph{dataframe}
...
leverages the \dask{} DataFrame \gls{api} for performing a query on
a dataframe loaded from a \gls{csv} file. The query is internally translated by \dask{} to the task
graph shown on the right side of the figure, which can then be executed e.g.\ on a distributed
cluster.
...
makes it easier to parallelise the computational workflow, yet it also increases the overall
...
parallelised and if it even makes sense to distribute it to multiple nodes at all. For example, if
...
StarPU~\cite{starpu}, Cilk~\cite{cilk}, TBB~\cite{tbb} or
\gls{openmp}~\cite{openmp}\footnote{Note that even though \gls{openmp} has been previously presented as an example of an
explicitly parallel model, it also offers a task system that provides facilities for implicit parallelisation.}. In these models, which are
sometimes called \emph{task parallel}, tasks usually represent either functions or blocks of
code that contain a handful of instructions, and are typically relatively quick to execute (they
can run e.g.\ for milliseconds or seconds). While some of these tools also support distributed
computing, their main use-case is to provide intra-node parallelisation on multicore systems with
shared memory, and often also to offload computation to attached accelerators, such as
\glspl{gpu} or \glspl{fpga}.

They can be used e.g.\ to implement high-performance numerically intensive computational kernels,
such as matrix multiplication or QR factorization~\cite{qr_factorization}, or to parallelise
...
primarily designed for executing complex scientific workflows, therefore they are not the primary
focus of this thesis. Instead, we will mainly examine task-based programming models that are
designed for distributed execution on a cluster, with tasks whose duration typically spans from
...
- task runtime vs workflow management systems
