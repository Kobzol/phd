...
\vspace{2mm}\makedef{def:task_graph} A task graph is a tuple $G = (T, O, E, \setresourcekindstaskgraph, \fntaskres)$, where:
...
%TODO: move this into the definition above?
$(T \cup O, E)$ forms a finite directed acyclic graph. The absence of cycles is important;
...
\begin{itemize}[itemsep=0pt]
	...
	\item $\fntaskworkerassigned$ is a function that returns the worker that was currently
	      ...
	      We assume that each task was executed at least once: $\forall t\in{}T, \exists tp\in\timedomain\colon \fntaskworkerassigned(t, tp) \neq \bot$
...
It could be generalized for tasks that can execute on multiple workers at once; however, that would
have to be performed in the context of a specific task runtime, as the semantics of
...
Unless a task runtime provides some special semantics, then the execution $E$
...
\begin{itemize}
	...
	\item[\makedef{def:worker_resource_constraint}] Resource requirements of all tasks must be fulfilled and resources provided by individual workers
		...
		This definition both satisfies all task resource requirements (which we will label as the
		\emph{task resource constraint}) and it makes sure that resources of workers are not oversubscribed at any
		given time (which we will label as the \emph{worker resource constraint}).
\end{itemize}
...
\vspace{2mm}\makedef{def:makespan} The makespan $M_E$ of a specific
task graph execution $E = ((T, O, E, \setresourcekinds, \fntaskres), C, \fntaskworkerassigned)$ is the duration between the start of the execution of
the first task to the completion of all tasks: \\ $M_E = \max\limits_{t \in T}(\fntaskfinish_E(t)) - \min\limits_{t \in T}(\fntaskstart_E(t))$
