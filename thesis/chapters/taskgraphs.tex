...
%TODO: function for getting vertices of a graph
\vspace{2mm}\makedef{def:task_graph}{Task graph} A task graph is a tuple
...
will then examine if that execution has satisfied various properties.

%TODO: infinite task durations?
...
Note that this definition assumes that each task in $E$ was being executed on at most a
...
Next, we will also define the following helper functions that will be used in later definitions:
...
until $t_1$ has finished executing. We assume that executions performed by all task runtimes that
will be further discussed in this thesis will always uphold this constraint.
