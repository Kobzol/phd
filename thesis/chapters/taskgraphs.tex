...
to resource management will be further extended in~\Autoref{ch:hyperqueue}.
...
in~\Autoref{sec:task-graph-execution}.
...
Additionally, tasks can also read and modify the state of the environment in
...
Below, we will provide a basic definition of task graph execution (and other concepts required
...
\vspace{2mm}\makedef{def:cluster} A \emph{computational environment} is a tuple $C = (W, \setresourcekindscluster, \fnworkerres)$, where:
\begin{itemize}[itemsep=0pt]
...
%TODO: how to use inner variables (G, C)?
\vspace{2mm}\makedef{def:task_graph_execution} An \emph{execution of a task graph} is a tuple
...
%TODO: should this have a definition? How to reference it?
...
\vspace{2mm}\makedef{def:makespan} The makespan $M_E$ of a specific task graph execution
...
shows two executions of a simple task graph that demonstrate how a trivial change in the
used schedule can severely affect the resulting makespan. The figure contains a task graph with
four tasks and two
