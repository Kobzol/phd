...
simple implementation detail is crucial for implementing the scheduler, and it should be included in
...
\begin{description}[wide=0pt]
	...
	\item[Network models] The simple (labeled \emph{simple}) and max-min (labeled \emph{max-min}) network
		models were used, with bandwidth speeds ranging from \SI{32}{\mebi\byte}/s to \SI{8}{\gibi\byte}/s. For experiments that do
...
information mode and a Minimal Scheduling Delay of $0.1$s.

Note that the vertical axis of some charts presented in this section does not start at zero, as
the goal was to focus on the relative difference between different scheduling algorithms rather
than on the absolute makespan durations.
...
For example, if a task graph took \SI{100}{\second} to execute in \dask{} with
the \emph{blevel} scheduler, but \SI{110}{\second} with the \emph{ws}
