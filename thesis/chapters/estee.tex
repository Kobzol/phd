...
execution. Task graphs can be structured arbitrarily, with large amounts of different kinds of
tasks with diverse execution characteristics and resource requirements.
...
\begin{enumerate}
	...
	\item We have benchmarked several task schedulers from existing literature under various conditions,
	      ...
	      types of task graphs. All parts of the benchmark suite, including the task graphs, all source code
	      for the schedulers and the simulation environment and also all benchmark scripts, are provided in
	      an open and reproducible form.
\end{enumerate}
...
\begin{description}[wide=0pt]
	\item[Task graphs] The \estee{} graph generators were used to generate a collection of task graphs that
		...
		in~\Autoref{tab:estee-graph-properties}. The generated task graph dataset is available as a reproducible
