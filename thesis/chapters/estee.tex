...
\glsxtrlong{msd}, the delay between two invocations of the scheduler and
...
the dependency constraint (\defref{def:dependency_constraint}) and both task and worker constraints
(\defref{def:worker_resource_constraint}).

The \emph{Worker} component simulates task execution and uses the provided network model to simulate exchanges of data (task
...
by~\defref{def:makespan}. In other words, our experiments estimate how quickly a given task
...
information mode and a \glsxtrlong{msd} of \SI{0.1}{\second}.
...
random scheduler holds up against more sophisticated approaches. \Autoref{fig:estee-chart-random-scheduler} compares
...
\begin{figure}
	...
	{\small horizontal axis: bandwidth [MiB/s]; vertical axis: makespan normalized to
	$\gls{msd}=0$}
	\caption{Comparison of \glsxtrshort{msd}; cluster 32x4}
...
significant than the effect of the \glsxtrlong{msd}.
...
in~\Autoref{sec:rsds-dask-overhead-analysis} and they can also be found in~\cite{rsds}. We have executed
these workflows with a $24x2$ cluster ($24$ cores on two nodes),
...
The \glsxtrlong{msd} had a smaller effect in our simulations than we have expected. This
