...
This thesis sets out to identify and analyze the existing challenges and bottlenecks, and design
...
designing parallelized programs on distributed clusters and introduces the task-based programming
model. \Autoref{ch:taskgraphs} then describes the task-based programming model in detail, by
defining key terms related to tasks and task graphs. It is followed by~\Autoref{ch:sota}, which
discusses state-of-the-art distributed task runtimes along with various ergonomic challenges and
performance bottlenecks of executing task graphs on \gls{hpc} systems.
The two subsequent chapters primarily focus on the performance aspects of task graph execution.
