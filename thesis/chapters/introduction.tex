...
task-based programming model in detail, by defining key terms related to tasks and task graphs. It
is followed by~\Autoref{ch:sota}, which discusses state-of-the-art distributed task runtimes
along with various ergonomic challenges and performance bottlenecks that they face when executing
task graphs on \gls{hpc} systems. The two subsequent chapters primarily focus on the
performance aspects of task graph execution. \Autoref{ch:estee} evaluates the performance of
various task scheduling algorithms and introduces \estee{}, a task graph execution
simulator. \Autoref{ch:rsds} then analyzes the runtime performance of \dask{},
a state-of-the-art task runtime, and proposes an alternative implementation of its server that is
able to outperform it in various \gls{hpc} use-cases. \Autoref{ch:hyperqueue} focuses
on improving both the ergonomics and performance aspects of task execution in the presence of
\gls{hpc} allocation managers, using a meta-scheduling design implemented by the
\hyperqueue{} task runtime. Finally,~\Autoref{ch:conclusion} summarizes the thesis and
