...
[Def.~\thetaskdefinition{}]\label{#1}}
\newcommand{\defref}[1]{\hyperref[#1]{Def.~\ref*{#1}}}

% Specific definitions
\newcommand{\fntaskres}{Res_t}
\newcommand{\fntaskresfrac}{\fntaskres^{frac}}
\newcommand{\fntaskresvariant}[1]{\fntaskres^{#1}}
\newcommand{\fnworkerres}{Res_w}
\newcommand{\fnworkerresident}{\fnworkerres^{id}}
\newcommand{\fntaskresassigned}{Res_a}
\newcommand{\fnworkertaskassigned}{E_w}
\newcommand{\fntaskstart}{T_s}
\newcommand{\fntaskfinish}{T_f}
\newcommand{\scheduleworker}{S_w}
\newcommand{\scheduleorder}{S_o}
\newcommand{\setresourcekinds}{R_k}
\newcommand{\setresourceelements}{R_e}
\newcommand{\powerset}[1]{\mathcal{P}(#1)}
\newcommand{\resourcerequirement}[2]{$#1\colon T \times \setresourcekinds \rightarrow \mathbb{#2}_{\geq{}0}$}
...
Hlavním přínosem práce je metoda pro plánování úloh, která umožňuje jednoduché spouštění úloh na superpočítačích,
které zároveň maximalizuje využití dostupných výpočetních zdrojů. Práce také poskytuje referenční implementaci
využívající této metody v rámci nástroje \hyperqueue{}, který usnadňuje spouštění grafů úloh na heterogenních
vysoce výkonných distribuovaných systémech. \hyperqueue{} je dostupné jako software s otevřeným zdrojovým kódem
pod licencí MIT (Massachusetts Institute of Technology) na adrese \url{https://github.com/it4innovations/hyperqueue}.}
\CzechKeywords{distribuované výpočty, výpočetní grafy, vysoce výkonné počítání}
